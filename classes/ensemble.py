...
class Ensemble:
  ...
  def __build_dataframe(self):
    """
    This method builds a dataframe containing all the predictions of all the considered
      models, multiplied by the accuracy of the model. We use this dataframe to do a weighted voting
      among the models.
    """
    ...
    # Explaination: we read each csv, using only the Prediction column. We then multiply that
    #   column for the valitation accuracy of the related model. We merge every csv in one single dataframe.
    ensemble_df = pd.concat([pd.read_csv(path, usecols=["Prediction"]).rename(columns = {"Prediction": f"Prediction{self.__model_names[i]}"}) \
      * validation_accuracies[i] for i, path in enumerate(paths)], axis = 1)
  ...
  def predict(self, submission_path):
    ...
    submission = pd.DataFrame(columns=['Id', 'Prediction'],
                              data={'Id': ensemble_df.index, 'Prediction': predictions})
